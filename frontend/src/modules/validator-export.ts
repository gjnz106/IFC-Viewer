...
// ── Toggle panel ──
window.toggleSGCheckPanel = function (): void {
  const panel = document.getElementById('sgPanel')!;
  const btn = document.getElementById('btnSGCheck')!;
  appState.sgState.open = !appState.sgState.open;
  btn.classList.toggle('active', (appState.sgState as any).open);

  if ((appState.sgState as any).open) {
...
window.sgExportReport = async function (): Promise<void> {
  if (!appState.sgState.results) { log('SG: no results to export'); return; }
  try {
    ...
    // KPIs
    const s = (appState.sgState.results as any).stats;
    ...
    for (const r of (appState.sgState.results as any).rules) {
...
window.sgExportBCF = async function (): Promise<void> {
  ...
  for (let fi = 0; fi < appState.files.length; fi++) {
    ...
    headerXml += '<File IfcProject="" IfcSpatialStructureElement="" isExternal="true"><Filename>' + escXml((appState.files[fi] as File).name) + '</Filename><Date>' + now + '</Date></File>';
  }
  headerXml += '</Header>';
  ...
  for (let i = 0; i < maxIssues; i++) {
    ...
    let modelIdx: number = iss.modelIdx;
    ...
    const bbox: any = (window as any).getElementBBox?.(modelIdx, iss.eid);
    ...
    if (bbox?.center) {
      ...
      try {
        ...
        if (mid !== undefined) {
          snapHL = (appState.ifcLoader as any).ifcManager.createSubset({ modelID: mid, ids: [iss.eid], material: hlMat, scene: appState.scene, removePrevious: false, customID: 'sgBcfSnap' });
          if (snapHL) { snapHL.position.copy((appState.loadedModels[modelIdx] as any).position); snapHL.updateMatrixWorld(true); }
        }
      } catch (e) {}
    ...
    try {
      const mid = (appState.loadedModels[modelIdx] as any)?.modelID;
      if (mid !== undefined) {
        const props = await (appState.ifcLoader as any).ifcManager.getItemProperties(mid, iss.eid, false);
